// File: chuong9.js
const vocabChuong9 = {
    // --- PHẦN CHÍNH ---
    "Chụp (ảnh), quay (phim) [Nhóm 1]": "撮る (とる)",
    "Chụp ảnh (cụm từ)": "写真を撮る (しゃしんをとる)",
    "Nóng (thời tiết: 暑い / đồ vật: 熱い)": "暑い / 熱い (あつい)",
    "Lạnh, rét (thời tiết)": "寒い (さむい)",
    "Ấm áp (thời tiết)": "暖かい (あたたかい)",
    "Mát mẻ (thời tiết)": "涼しい (すずしい)",
    "Vui": "楽しい (たのしい)",
    "Bận rộn": "忙しい (いそがしい)",
    "Rảnh rỗi": "暇 (ひま)",
    "Đơn giản": "簡単 (かんたん)",
    "Phức tạp": "複雑 (ふくざつ)",
    "Tuần trước nữa": "先々週 (せんせんしゅう)",
    "Tuần trước": "先週 (せんしゅう)",
    "Tuần này": "今週 (こんしゅう)",
    "Tuần tới": "来週 (らいしゅう)",
    "Tuần tới nữa": "再来週 (さらいしゅう)",
    "Sáng nay": "今朝 (けさ)",
    "Tối qua": "夕べ (ゆうべ)",
    "Ngày xưa": "昔 (むかし)",
    "Thời tiết": "天気 (てんき)",
    "Vùng, khu vực": "辺 (へん)",
    "Em bé": "赤ちゃん (あかちゃん)",
    "Bài kiểm tra": "テスト",
    "Bữa tiệc": "パーティー",
    "Nhà hàng": "レストラン",
    "Karaoke": "カラオケ",
    "Đã... rồi (phó từ hoàn thành - ví dụ: もう食べました) / Thêm nữa": "もう",
    "Vẫn, chưa (phó từ chưa hoàn thành - ví dụ: まだ食べていません)": "まだ",
    "Từ bây giờ": "これから・今から (これから・いまから)",
    "Cùng nhau": "一緒に (いっしょに)",
    "Lại, nữa (phó từ lặp lại - ví dụ: また会いましょう)": "また",

    // --- PHẦN MỞ RỘNG ---
    "Người lớn": "大人 (おとな)",
    "Trẻ con, con cái": "子供 (こども)",
    "Vé": "切符 (きっぷ)",
    "Việc ngắm hoa": "花見 (はなみ)",
    
    "Đuôi Đã - Động từ (Lịch sự / Văn nói)": "〜ました",
    "Đuôi Không đã - Động từ (Lịch sự / Văn nói)": "〜ませんでした",
    "Đuôi Đã - Động từ (Thân mật / Văn viết)": "〜た / 〜だ (tùy cách chia thể た)",
    "Đuôi Không đã - Động từ (Thân mật / Văn viết)": "〜なかった",

    // --- 2. DANH TỪ VÀ TÍNH TỪ ĐUÔI NA ---
    "Đuôi Đã - Danh từ & Tính từ na (Lịch sự / Văn nói)": "〜でした",
    "Đuôi Không đã - Danh từ & Tính từ na (Lịch sự / Văn nói)": "〜ではありませんでした",
    "Đuôi Đã - Danh từ & Tính từ na (Thân mật / Văn viết)": "〜だった",
    "Đuôi Không đã - Danh từ & Tính từ na (Thân mật / Văn viết)": "〜でなかった",

    // --- 3. TÍNH TỪ ĐUÔI I ---
    "Đuôi Đã - Tính từ i (Lịch sự / Văn nói)": "〜かったです",
    "Đuôi Không đã - Tính từ i (Lịch sự / Văn nói)": "〜くなかったです",
    "Đuôi Đã - Tính từ i (Thân mật / Văn viết)": "〜かった",
    "Đuôi Không đã - Tính từ i (Thân mật / Văn viết)": "〜くなかった"
};